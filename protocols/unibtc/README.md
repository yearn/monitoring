...
| PoR stale | `latestRoundData().updatedAt` | Older than 86,400s (Vault `mint()` reverts) | HIGH |
| Supply feeder wrong | Feeder `totalTokenSupply()` vs API `total_supply` | Gap > 2% | HIGH |
...
Pause, reserve-gate, PoR-staleness, feeder-gap, feeder-stale, and peg alerts fire once while the condition holds and re-arm on recovery. PoR coverage alerts on entering a worse band (HIGH then CRITICAL). Minting alerts fire per window when that run's delta crosses the threshold.
