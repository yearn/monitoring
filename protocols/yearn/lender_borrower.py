...
def _read_snapshot(config: StrategyConfig, *, include_rates: bool) -> StrategySnapshot:
    ...
    with client.batch_requests() as batch:
        ...
        if isinstance(config.market, MorphoMarketConfig):
            batch.add(strategy.functions.borrowUsdOracle())
        else:
            batch.add(strategy.functions.getLiquidateCollateralFactor())
    ...
    if isinstance(config.market, MorphoMarketConfig):
        ...
        with client.batch_requests() as batch:
            batch.add(morpho_oracle.functions.price())
            batch.add(price_feed.functions.decimals())
            batch.add(price_feed.functions.latestRoundData())
        ...
        with client.batch_requests() as batch:
            batch.add(price_oracle.functions.getAssetPrice(config.collateral_address))
            batch.add(price_oracle.functions.getAssetPrice(config.borrow_address))
    ...
    if include_rates:
        ...
        with client.batch_requests() as batch:
            ...
            if isinstance(config.market, MorphoMarketConfig):
                ...
                batch.add(morpho.functions.market(config.market.market_id))
            else:
                pool = client.get_contract(config.market.pool_address, AAVE_POOL_ABI)
                batch.add(pool.functions.getReserveData(config.borrow_address))
