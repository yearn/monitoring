...
def test_classify_collateral_band_boundaries() -> None:
    ...
    assert module.classify_collateral_band(Decimal("1.000099")) == module.ACCOUNTABLE_BAND_HIGH
    assert module.classify_collateral_band(Decimal("1.0001")) == module.ACCOUNTABLE_BAND_OK
    assert module.classify_collateral_band(Decimal("1.0002")) == module.ACCOUNTABLE_BAND_OK
...
def test_accountable_warning_band_alerts_high(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    module.check_accountable_collateral_band(make_accountable_report(module, "1.00005"))

    assert len(alerts) == 1
    assert alerts[0].severity == module.AlertSeverity.HIGH
    assert "100.0050%" in alerts[0].message
    assert "100.01% warning threshold" in alerts[0].message


def test_accountable_critical_alerts_immediately_below_100_percent(monkeypatch: pytest.MonkeyPatch) -> None:
    module = load_3jane_module()
    alerts: list = []
    stub_cache(monkeypatch, module)
    monkeypatch.setattr(module, "send_alert", alerts.append)

    module.check_accountable_collateral_band(make_accountable_report(module, "0.9999"))

    assert len(alerts) == 1
    assert alerts[0].severity == module.AlertSeverity.CRITICAL
    assert "undercollateralized" in alerts[0].message
...
def test_accountable_does_not_realert_within_same_band(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    module.check_accountable_collateral_band(make_accountable_report(module, "1.00009"))
    module.check_accountable_collateral_band(make_accountable_report(module, "1.00005"))
    module.check_accountable_collateral_band(make_accountable_report(module, "1.00001"))
...
def test_accountable_recovery_rearms_band(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    module.check_accountable_collateral_band(make_accountable_report(module, "1.00005"))
    module.check_accountable_collateral_band(make_accountable_report(module, "1.0002"))
    module.check_accountable_collateral_band(make_accountable_report(module, "1.00005"))
...
def test_accountable_stale_report_still_evaluates_ratio(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert module.AlertSeverity.CRITICAL in severities  # immediate sub-100% alert
