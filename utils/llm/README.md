...
3. Fetches the verified record of **both** implementations (`utils/verified_contract.py`) and diffs them (`utils/impl_diff.py`) in four separated categories, each line carrying the contract and file it came from:
   ...
   - **Storage compatibility** — `COMPATIBLE` / `INCOMPATIBLE` / `UNKNOWN` from the compiler `storageLayout` Sourcify publishes for both implementations (`utils/sourcify_layout.py`, `utils/storage_layout.py`). Slot, byte offset and recursive type shape are compared, including a normalized type *kind* at every level — `uint256` and `bytes32` fill the same 32 bytes, but retyping one as the other changes what stored data means, so it's `INCOMPATIBLE` ("reinterpreted as"). Contract types and `address` count as one kind, since that retype is intentional. Compiler-internal type/AST ids and variable names are ignored, so a rename at an unchanged slot is reported for context rather than flagged. Reserved `__gap` space may be consumed freely — what must hold is that every other variable keeps its slot.
   - **ERC-7201 namespaces** (`utils/namespaced_storage.py`) never appear in the positional layout, so `COMPATIBLE` also requires every namespace the deployed contract *or its inheritance chain* declares to be provably unchanged: same id, identical struct text, elementary member types only. The annotation sits on the struct inside the contract — usually an inherited base such as OpenZeppelin's `Initializable` — not on the contract declaration. Anything else (added, removed, edited, ambiguous, or user-defined member types) drops the verdict to `UNKNOWN` with the positional detail kept. Namespaces reached through merely-imported libraries, or declared without the annotation, are not checked and are listed as such.
...
utils/namespaced_storage.py  # ERC-7201 namespaces across the inheritance chain, proven unchanged or not
