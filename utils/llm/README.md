...
3. Fetches the verified record of **both** implementations (`utils/verified_contract.py`) and diffs them (`utils/impl_diff.py`) in four separated categories, each line carrying the contract and file it came from:
   ...
   - **Changed function bodies** (`utils/solidity_text.py`) — functions whose signature is unchanged but whose code is not, scoped to the deployed contract's own definition in its own file, with a short unified diff. ABI equality is not behavioral equality: this is what catches a proxy upgrade that rewrites everything behind an identical surface.
   - **Storage compatibility** — `COMPATIBLE` / `INCOMPATIBLE` / `UNKNOWN` from the compiler `storageLayout` Sourcify publishes for both implementations (`utils/sourcify_layout.py`, `utils/storage_layout.py`). Slot, byte offset and recursive type shape are compared; compiler-internal type/AST ids and variable names are not, so a rename at an unchanged slot is reported for context rather than flagged. Reserved `__gap` space may be consumed freely — what must hold is that every other variable keeps its slot.
